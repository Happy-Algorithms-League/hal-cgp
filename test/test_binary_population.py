...
def test_mutation_rate_within_bounds():
    ...
    with pytest.raises(ValueError):
        gp.BinaryPopulation(1, mutation_rate, SEED, {"genome_length": 2, "primitives": [0]})
...
def test_binary_population():
    ...
    genome_params = {"genome_length": 10, "primitives": list(range(10))}

    np.random.seed(SEED + 123)

    target_sequence = list(
        np.random.choice(genome_params["primitives"], size=genome_params["genome_length"])
    )
    ...
    def objective(individual):
        ...
        individual.fitness = sum(
            0 if individual.genome[i] == target_sequence[i] else -1
    ...
    ea = gp.ea.MuPlusLambda(
        params["n_offsprings"], params["n_breeding"], params["tournament_size"]
    )
